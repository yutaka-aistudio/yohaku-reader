const CACHE='yohaku-shell-02dd5d98b87a2d0b';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(fetch(event.request,{cache:'no-cache'}).then(r=>{if(!r.ok)throw new Error('offline');return r;}).catch(()=>caches.match(event.request).then(r=>r|| (event.request.mode==='navigate'?caches.match(new URL('./index.html',self.registration.scope).href):Response.error()))));
});
